
#pragma once

#include "Color.h"
#include "Rectangle.h"
#include <memory>

// Forward declarations for Skia
class SkPaint;
class SkShader;

namespace System {
namespace Drawing {

// Forward declarations
class Image;
class Matrix;

// Base brush class
class Brush {
public:
    virtual ~Brush() = default;
    virtual std::unique_ptr<Brush> Clone() const = 0;
    virtual void ApplyToSkPaint(SkPaint& paint) const = 0;
};

// Solid color brush
class SolidBrush : public Brush {
private:
    Color _color;
    
public:
    explicit SolidBrush(const Color& color);
    
    Color GetColor() const { return _color; }
    void SetColor(const Color& color) { _color = color; }
    
    std::unique_ptr<Brush> Clone() const override;
    void ApplyToSkPaint(SkPaint& paint) const override;
};

// Texture brush
class TextureBrush : public Brush {
private:
    std::shared_ptr<Image> _image;
    RectangleF _dstRect;
    
public:
    explicit TextureBrush(std::shared_ptr<Image> image);
    TextureBrush(std::shared_ptr<Image> image, const RectangleF& dstRect);
    
    std::shared_ptr<Image> GetImage() const { return _image; }
    
    std::unique_ptr<Brush> Clone() const override;
    void ApplyToSkPaint(SkPaint& paint) const override;
};

// Hatch brush patterns
enum class HatchStyle {
    Horizontal = 0,
    Vertical = 1,
    ForwardDiagonal = 2,
    BackwardDiagonal = 3,
    Cross = 4,
    DiagonalCross = 5,
    Percent05 = 6,
    Percent10 = 7,
    Percent20 = 8,
    Percent25 = 9,
    Percent30 = 10,
    Percent40 = 11,
    Percent50 = 12,
    Percent60 = 13,
    Percent70 = 14,
    Percent75 = 15,
    Percent80 = 16,
    Percent90 = 17,
    LightDownwardDiagonal = 18,
    LightUpwardDiagonal = 19,
    DarkDownwardDiagonal = 20,
    DarkUpwardDiagonal = 21,
    WideDownwardDiagonal = 22,
    WideUpwardDiagonal = 23,
    LightVertical = 24,
    LightHorizontal = 25,
    NarrowVertical = 26,
    NarrowHorizontal = 27,
    DarkVertical = 28,
    DarkHorizontal = 29,
    DashedDownwardDiagonal = 30,
    DashedUpwardDiagonal = 31,
    DashedHorizontal = 32,
    DashedVertical = 33,
    SmallConfetti = 34,
    LargeConfetti = 35,
    ZigZag = 36,
    Wave = 37,
    DiagonalBrick = 38,
    HorizontalBrick = 39,
    Weave = 40,
    Plaid = 41,
    Divot = 42,
    DottedGrid = 43,
    DottedDiamond = 44,
    Shingle = 45,
    Trellis = 46,
    Sphere = 47,
    SmallGrid = 48,
    SmallCheckerBoard = 49,
    LargeCheckerBoard = 50,
    OutlinedDiamond = 51,
    SolidDiamond = 52
};

// Hatch brush
class HatchBrush : public Brush {
private:
    HatchStyle _hatchStyle;
    Color _foreColor;
    Color _backColor;
    
public:
    HatchBrush(HatchStyle hatchStyle, const Color& foreColor);
    HatchBrush(HatchStyle hatchStyle, const Color& foreColor, const Color& backColor);
    
    HatchStyle GetHatchStyle() const { return _hatchStyle; }
    Color GetForegroundColor() const { return _foreColor; }
    Color GetBackgroundColor() const { return _backColor; }
    
    std::unique_ptr<Brush> Clone() const override;
    void ApplyToSkPaint(SkPaint& paint) const override;
    
private:
    sk_sp<SkShader> CreateHatchShader() const;
};

}
}
